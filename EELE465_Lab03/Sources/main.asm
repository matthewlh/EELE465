...
;*** write external temp in K
			...
			LDA		temp
			CMP		#$1B
			BLO		mainloop_external_k_small
			
mainloop_external_k_big:
			; convert to K
			SUB		#$1B
			STA		temp_k
			...
			LDA		#'3'
			JSR		lcd_char
			BRA		mainloop_external_k
			
mainloop_external_k_small:
			; convert to K
			ADD		#$49
			STA		temp_k
...
;*** do math on internal temp sensor data

			; divide adc_data by n
			LDHX	adc_data_0
			LDA		adc_data_1
			LDX		num_samples
			DIV						; A <= (H:A)/(X), H <= (remainder)
